...
public class MyPlayList {
	...
	public static void main(String[] args) {
		...
		ArrayList<String> playlist = new ArrayList<>(Arrays.asList("비의 랩소디:임재현", "Perfect Night:LE SSERAFIM (르세라핌)",
				...
				"편지 한 장 (부제: 서른에 만난 첫 세상) (A letter):김호중", "I Just Love Ya:PLAV"));
		
		Scanner scan = new Scanner(System.in);
		HashMap<String, ArrayList<String>> userMap = new HashMap<>();
		System.out.println(makeSongList(playlist));
		...
		if(singer.equals("q")){
			...
			break;
		      ...
		      ArrayList<String> searchList = searchSong(singer, playlist);
		      Collections.sort(searchList);
		      System.out.println(searchList);}
		System.out.println("어떤 노래를 저장하시나요?(이전: n, 종료: q)");
		System.out.println(">>> ");
...
//        	System.out.println(students.get(i));
        	if(playlist.contains(singer)) {
        	 System.out.println("김영주 님의 출석 번호: "+(playlist.indexOf()+1));
        		break;
	...
	public static ArrayList<String> searchSong(String singer, ArrayList<String> arr) {
		ArrayList<String> result = new ArrayList<>();
		for (int i = 0; i < arr.size(); i++) {
			// :을 기준으로 자르기 0 제목 1 가수
			String[] temp = arr.get(i).split(":");{
				if (temp[1].contains(singer)) {
	...
	public static HashSet<String> makeSongList(HashSet<String> arr) {
